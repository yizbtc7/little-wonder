...
export async function POST(request: Request, context: { params: Promise<{ childId: string }> }) {
  try {
    ...
    let uploadResult = await db.storage.from(PHOTO_BUCKET).upload(objectPath, file, {
      cacheControl: '3600',
      contentType: file.type,
      upsert: false,
    });

    if (uploadResult.error && /bucket/i.test(uploadResult.error.message)) {
      await db.storage.createBucket(PHOTO_BUCKET, { public: true, fileSizeLimit: `${MAX_FILE_SIZE_BYTES}` });
      uploadResult = await db.storage.from(PHOTO_BUCKET).upload(objectPath, file, {
        cacheControl: '3600',
        contentType: file.type,
        upsert: false,
      });
    ...
    if (uploadResult.error) {
      return NextResponse.json({ error: uploadResult.error.message || 'Could not upload photo right now. Please try again.' }, { status: 500 });
    }
