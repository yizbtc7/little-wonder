...
function cleanWonderSectionText(text: string): string {
  return text
    .replace(/^\s*(?:🤲|🧡)?\s*(?:CÓMO\s+ESTAR\s+PRESENTE|Cómo\s+estar\s+presente|How\s+to\s+be\s+present)\s*[:\-–—]?\s*/i, '')
    .trim();
}
  ...
  return (
    <main style={{ position: 'relative', maxWidth: 390, margin: '0 auto', minHeight: '100vh', background: theme.colors.cream, boxShadow: '0 0 60px rgba(0,0,0,0.06)', display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ padding: '0 24px calc(124px + env(safe-area-inset-bottom))' }}>
                ...
                {String(openWonder.article.how_to_be_present ?? '').trim() ? (
                  <div style={{ background: `linear-gradient(135deg, ${theme.colors.blush} 0%, ${theme.colors.warmWhite} 100%)`, borderRadius: 24, padding: '24px 22px' }}>
                    ...
                    <p style={{ margin: 0, fontFamily: theme.fonts.sans, fontSize: 16, lineHeight: 1.7, color: theme.colors.darkText }}>{cleanWonderSectionText(openWonder.article.how_to_be_present)}</p>
                  </div>
                ) : null}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: 8, marginTop: 16, position: 'relative', zIndex: 1 }}>
                  {[
                    ...
                    <div key={s.l} style={{ aspectRatio: '1 / 1', background: 'rgba(255,255,255,0.66)', borderRadius: 14, border: '1px solid rgba(255,255,255,0.76)', padding: '12px 6px 10px', textAlign: 'center', boxShadow: '0 2px 8px rgba(93,70,63,0.06)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                      <p style={{ margin: 0, minHeight: 24, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: theme.fonts.display, fontSize: 24, lineHeight: 1, fontWeight: 400, fontVariantNumeric: 'tabular-nums', color: '#43322E' }}>{s.n}</p>
                      <p style={{ margin: '6px 0 0', minHeight: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: theme.fonts.sans, fontSize: 11, lineHeight: 1.1, color: '#755F59', fontWeight: 700 }}>{s.l}</p>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <p style={{ margin: '10px 0 94px', textAlign: 'center', fontFamily: theme.fonts.sans, fontSize: 11, color: 'rgba(123,115,113,0.78)' }}>
        {locale === 'es' ? 'La curiosidad de tu hijo es su superpoder. Te ayudamos a verla, entenderla y nutrirla.' : "Your child’s curiosity is their superpower. We help you see it, understand it, and nurture it."}
      </p>
